...
//
//func initOptions() *types.Options {
//	var options = &types.Options{}
//	flagSet := goflags.NewFlagSet()
//	flagSet.CaseSensitive = true
//	flagSet.SetDescription(`x`)
//
...
func (s *Nuclei) StartV3(task *entity.NodeTask, scannerPath, templatesDir string, targets []string, output string) error {
	u0, _ := url.Parse(task.Target)
	taskScheme := u0.Scheme
	...
	for _, v := range targets {
		u, _ := url.Parse(v)
		if u.Scheme == "http" {
			httpScheme = append(httpScheme, v)
		}
		if u.Scheme == "https" {
			httpsScheme = append(httpsScheme, v)
		}
	}

	var tmp []string
	if taskScheme == "http" {
		tmp = append(tmp, httpScheme...)
		if len(httpsScheme) > 1 {
			tmp = append(tmp, httpsScheme...)
		}
	}
	...
	for _, t := range targets {
		_targets = append(_targets, "-u "+t)
	}
	//cmdName := scannerPath
	cmdArgs := []string{
		//"-list", targets,
		//"-target", strings.Join(_targets, " "),
		//"-t", e.Templates[0],
		"-jsonl-export", output,
		"-no-interactsh",
		"-rate-limit", strconv.Itoa(1000),
		"-bulk-size", strconv.Itoa(250),
		"-concurrency", strconv.Itoa(250),
	...
	cmdStr = scannerPath + " " + strings.Join(_targets, " ") + " " + cmdStr

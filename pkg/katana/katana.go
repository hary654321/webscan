...
//
// )
type KatanaInterface interface {
	Start(task *entity.SlaveTask, output string) error
	Stop()
}
...
var _ KatanaInterface = &Katana{}

func (k *Katana) Start(task *entity.SlaveTask, output string) error {
	myOptions, err := initOptions()
	if err != nil {
		gologger.Fatal().Msgf("Could not read flags: %s\n", err)
	}
	// 根据task的参数设置myOptions
	myOptions.URLs = []string{task.Target}
	if task.CrawlScope != "." {
		myOptions.Scope = []string{task.CrawlScope}
	}
	myOptions.MaxDepth = int(task.Depth)
	myOptions.DisplayOutScope = true
	myOptions.OutputFile = output
	//if myOptions.HealthCheck {
	//	gologger.Print().Msgf("%s\n", runner.DoHealthCheck(myOptions, flagSet))
	//	os.Exit(0)
	//}

	katanaRunner, err := runner.New(myOptions)
	...
	//}
	if err != nil {
		return errorutil.NewWithErr(err).Msgf("could not create runner")
	}
...
func initOptions() (*types.Options, error) {
